     ...
     <div className="banner bg-white py-10 lg:py-20 overflow-hidden">
        <div className="banner-Container mx-auto max-w-7xl px-4 lg:px-20 flex flex-col lg:flex-row items-center justify-between gap-10">
          <div className="banner-Content flex-1 text-center lg:text-left">
            ...
            <h2 className='font-extrabold text-3xl md:text-5xl lg:text-6xl text-[#101727] leading-tight'>
              Supercharge Your <br className="hidden md:block" /> Digital Workflow
            </h2>

            <p className='text-gray-500 my-6 text-sm md:text-lg'>
              Access premium AI tools, design assets, templates, and productivity <br className="hidden lg:block" />
              software—all in one place. Start creating faster today.
            </p> 
            
            <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4">
              <a className="btn rounded-full px-8 h-12 w-full sm:w-52 text-white border-none font-semibold text-lg bg-linear-to-r from-[#4F39F6] to-[#b23fc5]">
                Explore Products
              </a>
              <a className="flex items-center justify-center h-12 w-full sm:w-52 gap-2 px-8 py-3 rounded-full border border-[#4F39F6] font-semibold text-lg text-[#4F39F6] hover:bg-purple-50 transition-all cursor-pointer">
          ...
          <div className="bannerImage flex-1 w-full max-w-lg lg:max-w-none">
     ...
     <div className="w-full bg-linear-to-r from-[#4F39F6] to-[#b23fc5] py-10 md:py-14 text-white">
        <div className="flex flex-col md:flex-row text-center max-w-6xl mx-auto items-center gap-8 md:gap-0">
          <div className="flex-1">
            ...
            <p className="text-lg md:text-xl opacity-80 mt-2">Active Users</p>
          </div>
          <div className="flex-1 w-full py-4 border-y md:border-y-0 md:border-x border-white/20">
            <h2 className="text-4xl md:text-5xl font-bold">200+</h2>
            <p className="text-lg md:text-xl opacity-80 mt-2">Premium Tools</p>
          </div>
          <div className="flex-1">
            <h2 className="text-4xl md:text-5xl font-bold">4.9</h2>
            <p className="text-lg md:text-xl opacity-80 mt-2">Rating</p>
          </div>
        </div> 
      </div>
        ...
        <div className="digital-tools w-full py-16 md:py-[120px] px-4 md:px-[50px] lg:px-[100px]">
          <div className="digital-tools-Container flex flex-col items-center text-center">
            <div className="digital-tools-Content max-w-2xl">
              ...
              <p className="text-base md:text-lg text-[#0F172A]/70 mb-8 px-2">
              ...
              <div className="digital-tools-actions mt-6">
                <div className="inline-flex items-center p-1 bg-white border border-gray-200 rounded-full shadow-sm">    
                  <button 
                    ...
                    className={`px-6 md:px-10 py-2 md:py-3 rounded-full font-semibold text-sm md:text-lg transition-all ${activeTab === 'products' ? 'bg-linear-to-r from-[#4F39F6] to-[#b23fc5] text-white' : 'text-[#0F172A]'}`}
                  >
                    Products
                  </button>
                  <button 
                    onClick={() => setActiveTab('cart')}
                    className={`px-6 md:px-10 py-2 md:py-3 rounded-full font-semibold text-sm md:text-lg transition-all ${activeTab === 'cart' ? 'bg-linear-to-r from-[#4F39F6] to-[#b23fc5] text-white' : 'text-[#0F172A]'}`}
                  >
                    Cart ({cart.length})
                  </button>    
                </div>
            ...
            <div className="digital-tools-Card mt-12 md:mt-16 w-full max-w-[1100px] mx-auto">
              {activeTab === 'products' ? (
                    ...
                    return (
                      <div key={product.id} className="border border-gray-100 rounded-[32px] p-6 text-left relative shadow-sm transition-all flex flex-col bg-[#FFFFFF] min-h-[420px]">
                        ...
                        <div className="w-10 h-10 flex items-center justify-center rounded-full bg-purple-50 mb-4 overflow-hidden border border-gray-100">
                          {product.id === 3 ? <span className="material-symbols-outlined text-xl text-[#4F39F6]">{product.icon}</span> : <img src={product.img} alt="" className="w-6 h-6 object-contain" />}
                        </div>
                        ...
                        <button onClick={() => addToCart(product)} className={`w-full py-3 rounded-full font-bold transition-all text-sm text-white ${isAlreadyInCart ? 'bg-green-500' : 'bg-linear-to-r from-[#4F39F6] to-[#b23fc5] active:scale-95'}`}>
                ...
                <div className="max-w-2xl mx-auto py-10 px-4 md:px-6 border border-dashed border-gray-300 rounded-3xl bg-gray-50 text-center">
                  {cart.length === 0 ? (
                    <div className="py-10"><span className="material-symbols-outlined text-6xl text-gray-200 mb-4">shopping_cart</span><p className="text-gray-400">Your cart is empty</p></div>
                  ) : (
      ...
      <div className="steps w-full py-16 md:py-[120px] px-6 lg:px-[200px] bg-gray-50 font-poppins">
        <div className="steps-Container flex flex-col items-center">
          ...
          <div className="steps-Card grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl">
            <div className="card-item relative bg-white border border-gray-100 rounded-[2rem] p-8 md:p-10 flex flex-col items-center text-center shadow-lg">
              ...
              <div className="w-20 h-20 bg-purple-50 rounded-full flex items-center justify-center mb-6 md:mb-8"><img src={userIcon} alt="User" className="w-10 h-10 object-contain" /></div>
              <h3 className="text-xl md:text-2xl font-bold text-[#0F172A] mb-4">Create Account</h3>
              <p className="text-gray-500 text-xs md:text-sm leading-relaxed max-w-[220px]">Sign up for free in seconds. No credit card required.</p>
            </div>
            <div className="card-item relative bg-white border border-gray-100 rounded-[2rem] p-8 md:p-10 flex flex-col items-center text-center shadow-lg">
              ...
              <div className="w-20 h-20 bg-purple-50 rounded-full flex items-center justify-center mb-6 md:mb-8"><img src={packageIcon} alt="Package" className="w-10 h-10 object-contain" /></div>
              <h3 className="text-xl md:text-2xl font-bold text-[#0F172A] mb-4">Choose Products</h3>
              <p className="text-gray-500 text-xs md:text-sm leading-relaxed max-w-[220px]">Browse our premium catalog and select your tools.</p>
            </div>
            <div className="card-item relative bg-white border border-gray-100 rounded-[2rem] p-8 md:p-10 flex flex-col items-center text-center shadow-lg">
              ...
              <div className="w-20 h-20 bg-purple-50 rounded-full flex items-center justify-center mb-6 md:mb-8"><img src={rocketIcon} alt="Rocket" className="w-10 h-10 object-contain" /></div>
              <h3 className="text-xl md:text-2xl font-bold text-[#0F172A] mb-4">Start Creating</h3>
              <p className="text-gray-500 text-xs md:text-sm leading-relaxed max-w-[220px]">Download and start using your tools immediately.</p>
            </div>
          </div>
        </div>
      </div> 
      ...
      <div className="price w-full py-16 md:py-[120px] px-4 md:px-[50px] lg:px-[200px] bg-[#FFFFFF] font-poppins">
        <div className="price-Container flex flex-col items-center">
          ...
          <div className="flex flex-col md:flex-row justify-center items-stretch gap-8 font-poppins w-full max-w-6xl">
            {/* Starter */}
            <div className="price-card w-full md:w-[300px] bg-gray-100 border border-gray-100 rounded-[2rem] p-8 shadow-lg flex flex-col text-left">
              <h3 className="text-xl font-bold text-[#0F172A] mb-1">Starter</h3>
              <p className="text-gray-600 text-[11px] mb-6">Perfect for getting started</p>
              <div className="flex items-baseline mb-6"><span className="text-4xl font-bold text-[#0F172A]">$0</span><span className="text-xs font-medium text-gray-600 ml-1">/Month</span></div>
              <ul className="space-y-3 mb-10 flex-grow">
                <li className="flex items-center text-gray-600 text-xs"><span className="material-symbols-outlined text-green-500 mr-2 text-base">check</span>10 free tools</li>
                <li className="flex items-center text-gray-600 text-xs"><span className="material-symbols-outlined text-green-500 mr-2 text-base">check</span>Basic templates</li>
              </ul>
              <button className="w-full py-3 bg-linear-to-r from-[#4F39F6] to-[#b23fc5] text-white rounded-full font-bold text-sm">Get Started</button>
            </div>
            {/* Pro */}
            <div className="price-card relative w-full md:w-[320px] bg-linear-to-r from-[#4F39F6] to-[#b23fc5] p-8 pt-12 rounded-[2rem] shadow-2xl flex flex-col text-left text-white transform md:scale-105 z-10">
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#FFEFD5] text-[#BB4D00] text-[10px] font-bold px-4 py-1 rounded-full uppercase tracking-wider shadow-md">Most Popular</div>
              <h3 className="text-xl font-bold mb-1">Pro</h3>
              <p className="text-white opacity-80 text-[11px] mb-6">Best for professionals</p>
              <div className="flex items-baseline mb-6"><span className="text-4xl font-extrabold">$29</span><span className="text-xs font-medium opacity-80 ml-1">/Month</span></div>
              <ul className="space-y-3 mb-10 flex-grow">
                <li className="flex items-center text-[12px]"><span className="material-symbols-outlined text-white mr-2 text-base">check</span>All premium tools</li>
                <li className="flex items-center text-[12px]"><span className="material-symbols-outlined text-white mr-2 text-base">check</span>Unlimited projects</li>
              </ul>
              <button className="w-full py-3 bg-white text-[#4F39F6] rounded-full font-bold text-sm">Start Trial</button>
            </div>
            {/* Enterprise */}
            <div className="price-card w-full md:w-[300px] bg-gray-100 border border-gray-100 rounded-[2rem] p-8 shadow-lg flex flex-col text-left">
              <h3 className="text-xl font-bold text-[#0F172A] mb-1">Enterprise</h3>
              <p className="text-gray-600 text-[11px] mb-6">For teams</p>
              <div className="flex items-baseline mb-6"><span className="text-4xl font-bold text-[#0F172A]">$99</span><span className="text-xs font-medium text-gray-600 ml-1">/Month</span></div>
              <ul className="space-y-3 mb-10 flex-grow">
                <li className="flex items-center text-gray-600 text-xs"><span className="material-symbols-outlined text-green-500 mr-2 text-base">check</span>Team collaboration</li>
                <li className="flex items-center text-gray-600 text-xs"><span className="material-symbols-outlined text-green-500 mr-2 text-base">check</span>Dedicated support</li>
              </ul>
              <button className="w-full py-3 bg-linear-to-r from-[#4F39F6] to-[#b23fc5] text-white rounded-full font-bold text-sm">Contact Sales</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="workflow w-full py-16 bg-linear-to-r from-[#4F39F6] to-[#b23fc5] text-white font-poppins px-6">
        <div className="workflow-Container max-w-4xl mx-auto flex flex-col items-center text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6 w-full sm:w-auto">
            <button className="px-8 py-3 bg-white text-[#4F39F6] rounded-full font-bold">Explore Products</button>
            <button className="px-8 py-3 bg-transparent border-2 border-white rounded-full font-bold">View Pricing</button>
          </div>
        </div>
      </div> 
